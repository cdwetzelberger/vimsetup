...
" Identifiers
syn match rubyLocalVariableOrMethod "\<[_[:lower:]][_[:alnum:]]*[?!=]\=" contains=@Spell display transparent
syn match rubyBlockArgument	    "&[_[:lower:]][_[:alnum:]]"		 contains=@Spell display transparent

syn match  rubyConstant		"\%(\%([.@$]\@<!\.\)\@<!\<\|::\)\_s*\zs\u\w*\%(\>\|::\)\@=\%(\s*(\)\@!" contains=@Spell
syn match  rubyClassVariable	"@@\h\w*" contains=@Spell display
syn match  rubyInstanceVariable "@\h\w*"  contains=@Spell display
syn match  rubyGlobalVariable	"$\%(\h\w*\|-.\)" contains=@Spell
syn match  rubySymbol		"[]})\"':]\@<!:\%(\^\|\~\|<<\|<=>\|<=\|<\|===\|==\|=\~\|>>\|>=\|>\||\|-@\|-\|/\|\[]=\|\[]\|\*\*\|\*\|&\|%\|+@\|+\|`\)" contains=@Spell
syn match  rubySymbol		"[]})\"':]\@<!:\$\%(-.\|[`~<=>_,;:!?/.'"@$*\&+0]\)" contains=@Spell
syn match  rubySymbol		"[]})\"':]\@<!:\%(\$\|@@\=\)\=\h\w*" contains=@Spell
syn match  rubySymbol		"[]})\"':]\@<!:\h\w*\%([?!=]>\@!\)\=" contains=@Spell
syn match  rubySymbol		"\%([{(,]\_s*\)\@<=\l\w*[!?]\=::\@!"he=e-1 contains=@Spell
syn match  rubySymbol		"[]})\"':]\@<!\h\w*[!?]\=:\s\@="he=e-1 contains=@Spell
...
syn match  rubyBlockParameter	  "\h\w*" contained contains=@Spell
